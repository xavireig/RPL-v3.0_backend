...
class GameWeek < ApplicationRecord
  # change formation of the running round only
  class ChangeCurrentGwFormation < Trailblazer::Operation
    extend Contract::DSL
    ...
    failure :match_day_has_started, fail_fast: true
    step :update_game_week_formation!
    step :parse_formation
    step :perform_format
    success :update_bench_reserved
    # Uncommenting this will affect formation of next all game weeks
    ...
    def match_day_started?(options)
      options['model'].virtual_engagements.each do |ve|
        return false if
          ve.footballer.
              fixture_on_round(options['model'].virtual_round.round).ongoing?
      end
      true
    end

    def match_day_has_started(options)
      options['message'] = 'Match day has begun. Formations are locked!'
    end
