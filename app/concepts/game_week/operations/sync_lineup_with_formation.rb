...
class GameWeek < ApplicationRecord
  ...
  class SyncLineupWithFormation < Trailblazer::Operation
    extend Contract::DSL
    ...
    def take(v_footballers, num, round)
      # sort is desc order
      v_footballers.sort_by(&:rank).select{|v_f| !v_f.fixture_on_round(round).ongoing? }.first(num)      # TODO: uncomment .select{|v_f| !v_f.fixture_on_round(round).ongoing? }.first(num)
    end
